...
// 4. Update LCD screen to show more useful information. Perhaps, show selected effect name instead of FX1 on/off.
...
// these variables are used when an external MIDI clock
...
void setup() {
  ...
  Serial.begin(115200);
  //Serial.begin(31250);
  lcd1.begin (20, 4); //  LCD1 is 20x4 - for some reason Teensy 3.6 needs this line twice
  // delay(1000);
    ...
    if (footSw4.wasReleased()) {
      if (long_press_release < 1) {
        if (chnMode == 1) {
          ...
          else if (tapTimerMode > 0 && tapTimerMode < 3) {
            tapTime = millis() - tapTimerStart;
            totalTime = tapTime + totalTime;
            tapTimerStart = millis();
            tapTimerMode++;
          }

          else if (tapTimerMode == 3) {
            tapTime = millis() - tapTimerStart;
            totalTime = tapTime + totalTime;
            tempoMillis = totalTime / 3;
            tapTimerStart = millis();
